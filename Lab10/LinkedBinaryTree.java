...
public class LinkedBinaryTree<T> implements BinaryTreeADT<T> {
	...
	/**
	 * Returns the integer size of this subtree rooted at r.
	 *
	 * @return  the integer size of this subtree rooted at r.
	 */
	public int size(BinaryTreeNode<T> r) {
		if (r == null) {
			return 0; // Base case: an empty subtree has size 0
		}
		return 1 + size(r.getLeft()) + size(r.getRight()); // Recursive case
	}
	...
	public boolean contains(BinaryTreeNode<T> r, T targetElement) {
		if (r == null) {
			return false; // Base case: empty subtree does not contain the element
		}
	...
	protected void preorder(BinaryTreeNode<T> node, ArrayUnorderedList<T> tempList) {
		if (node != null) {
			tempList.addToRear(node.getData()); // Visit root first
			preorder(node.getLeft(), tempList); // Then left subtree
			preorder(node.getRight(), tempList); // Then right subtree
		}
	}
	...
	protected void postorder(BinaryTreeNode<T> node, ArrayUnorderedList<T> tempList) {
		if (node != null) {
			postorder(node.getLeft(), tempList); // Visit left subtree
			postorder(node.getRight(), tempList); // Visit right subtree
			tempList.addToRear(node.getData()); // Visit root last
		}
	}
